...
## 16. 빈 필수 필드가 파일을 깨뜨린 사고 (2026-08-23)

**증상:** 설명을 비운 채 저장한 글이 목록에 "오류"로 뜨고, 그다음부터 **dev 서버가 아예 뜨지
않았다.** `astro build`는 `[InvalidContentEntryDataError] description: Required`로 실패했다.

**원인 사슬 — 전부 관리자 쪽이다.**

1. 사용자가 설명 칸을 비우고 저장 → 클라이언트가 `description: ''` 전송
2. 스키마 `z.string().max(200)`은 **빈 문자열을 통과시킨다**
3. 직렬화의 `prune()`이 빈 문자열을 **키째로 생략**
4. 파일에 `description` 키가 없다 → 다시 읽을 때 `Required`로 실패

**CRITICAL: 이 사고의 성질은 "교착"이다.** 콘텐츠 컬렉션이 깨지면 dev 서버가 기동하지
못하고, 관리자는 dev 서버 안에 살기 때문에 **고칠 도구조차 열리지 않는다.**
그래서 이 클래스는 "저장 시 경고"로 충분하지 않고 **애초에 쓰이지 않게** 막아야 한다.
...
이제 행 안에 `필드: 메시지`로 직접 보여 준다.

**검증:** 빈 description 거절 · 공백뿐인 title 거절 · 파일 미생성 · 정상 저장 후 재읽기 valid ·
직렬화가 빈 필수 키를 남김 — 8건.

---

## 17. 이미지 표시 옵션 — 크기와 정렬 (사용자 요청)

사용자가 "이미지 크기를 조절하고 싶다"고 요청했다. 두 해석이 있어 물었고
**본문 표시 크기**(픽셀 직접 입력)로 확정했다. 업로드 원본 축소(스펙 Q3)는 채택하지 않았다 —
원본 화질을 되돌릴 수 없기 때문이다.

### 표기와 구현

alt 텍스트 **맨 끝**에 `|옵션`을 붙인다. `markdown.config.mjs`의 `rehypeImageSize`가
처리하므로 **빌드와 프리뷰가 자동으로 같다.**

| 입력 | 결과 |
| ---- | ---- |
| `![설명\|400](x.png)` | `max-width: min(400px, 100%)`, alt은 `설명` |
| `![설명\|250px](x.png)` | `px` 접미 허용 |
| `![설명\|center](x.png)` | `display: block; margin-inline: auto` |
| `![설명\|left](x.png)` | `margin-inline-end: auto` |
| `![설명\|right](x.png)` | `margin-inline-start: auto` |
| `![설명\|400\|center](x.png)` | 둘 다. **순서 무관** |
| `![설명](x.png)` | 원본 크기, 글 흐름 그대로 |
| `![A \| B](x.png)` | 마지막 조각이 아는 토큰이 아니라 그대로 둔다 |

**파싱 규칙:** 뒤에서부터 **아는 토큰인 동안만** 떼어 낸다(숫자 또는 정렬 키워드).
모르는 조각을 만나면 즉시 멈추고, `parts[0]`(설명문 본체)은 절대 소비하지 않는다.
그래야 파이프가 들어간 평범한 문장이 옵션으로 오해되지 않는다.

**CRITICAL: 정렬에는 `display: block`이 함께 필요하다.** 이미지는 기본이 인라인이라
`margin: auto`가 듣지 않는다.

**정렬은 `float`가 아니다.** 이미지 상자의 위치만 옮기며 글이 감싸고 흐르지는 않는다 —
"가운데/왼쪽/오른쪽 정렬"이라는 요청의 자연스러운 해석이고, float는 본문 리듬을 크게 바꾼다.

**CRITICAL: `width` 속성을 덮어쓰지 않는다.** Astro가 넣어 준 `width`/`height`는 원본 비율이며
브라우저가 로드 전에 자리를 잡는 근거다(CLS). 표시 크기는 `max-width`로만 줄인다.

**CRITICAL: `min(…px, 100%)`을 쓴다.** 고정 px는 좁은 화면에서 뷰포트를 넘겨 가로 스크롤을 만든다.

`unist-util-visit`를 쓰지 않고 여덟 줄짜리 재귀 순회를 직접 썼다 — 전이 의존성 호이스팅에
기대지 않는다(`yaml`을 명시 선언한 것과 같은 이유).

### 곁다리로 잡은 프리뷰 결함

본문 이미지가 **프리뷰에서 404로 깨져 보이고 있었다.** 저장되는 경로는 글 파일 기준
`../../../assets/x.png`인데, 관리자 페이지(`/admin/posts/edit`)에서 그 상대 경로는
`/assets/x.png`로 풀린다 — dev 서버가 서빙하지 않는 자리다(실측 404).
`preview.mjs`가 렌더 결과의 `<img src>`만 `/src/assets/…`로 바꾼다.
**파일에 저장되는 본문은 건드리지 않는다** — 저장된 경로는 빌드가 해석해야 하는 값이다.

---

## 18. 테스트 하네스 주의 — dev 서버 상태 누적

전체 검사(11묶음, 138건)를 **같은 dev 서버에서 연달아 두 번** 돌리면 두 번째 배치에서
글 페이지가 500을 내거나 카테고리 쓰기가 실패한다. 원인은 제품이 아니라 **오래 살아 있는
dev 서버가 반복적인 콘텐츠·설정 재작성으로 상태가 어긋나는 것**이다. 실측으로 확인했다.

| 조건 | 결과 |
| ---- | ---- |
| 같은 서버에서 배치 2회 연속 | 2회차에서 `preview-parity` 8/12, `write-test` 실패 |
| 같은 내용을 다른 슬러그로 저장 | 정상(200) — 내용 문제가 아니다 |
| 콘텐츠 동기화 지연 측정(5회) | 매번 약 1.15초로 일정 — 폴링 시간 문제가 아니다 |
| **배치마다 서버 재시작 (3회)** | **전부 통과** |
